...
# 特别注意：其他绘图命令在insert box之前写，因为绘图起点将被改变

# 最后在右上角绘制insert box,并利用-D的+t子选项把绘图起点移到insert box的左下角
gmt psbasemap -R -J -O -K -DjTR+w1.5i+o0.15i+t -F+gwhite+p1p+c0.1c+s >> $ps
# 在insert box内绘制全球地图，并突出显示澳大利亚。其中-J选项的1.5i表示insert box的宽度，由上句-D选项的+w决定
