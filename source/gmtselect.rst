...
``-Z<min>[/<max>][+a][+c<col>][+i]``
    ...
    若省略 ``<max>`` 则判断Z值是否等于 ``<min>`` 。若不限制范围的上限或下限，
    可以使用 ``-`` 代替。
    ...
    不同的列号，仅当所有测试都通过的行才会输出。注意：当多次使用 ``-N``
    选项时，不可使用 ``-Iz`` 选项。

    在多次重复使用 ``-Z`` 时：

    - ``+a`` 表示只要当前行通过任意一个测试即输出该行
    - ``+i`` 表示反转所有测试，即输出Z值不在指定范围内的记录
    - 若没有使用 ``+c`` 选项，则每次使用 ``-Z`` 选项分别作用于不同的列，
      从2（即第三列）开始。
