...
``-E[<bordervalues>]``
    默认情况下会将恰好位于多边形边界上的节点当作是在多边形的内部，使用 ``-E``
    则将其当作是在多边形的外部。

    可以使用 ``-E<bordervalue>`` 为被边界穿过的单元设置特定的值，也可以使用
    ``-E<cborder>/<lborder>/<iborder>/<pborder>`` 为被不同等级的边界数据所
    穿过的单元设置不同的值，其中四个值分别表示海岸线、湖岸线、湖中岛边界和
    湖中岛中湖的边界所对应的节点值。
